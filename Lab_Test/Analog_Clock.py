...
while running:
    ...
    # Draw numbers to indicate the hours on the clock
    for i in range(12):
        hour_angle = math.radians(90 - i * 30)  # Calculate the angle for each hour
        hour_x = center_x + (clock_radius * 0.8) * math.cos(hour_angle)
        hour_y = center_y - (clock_radius * 0.8) * math.sin(hour_angle)
        font = pygame.font.Font(None, 36)
        text = font.render(str(i + 1), True, (0, 0, 0))
        text_rect = text.get_rect(center=(hour_x, hour_y))
